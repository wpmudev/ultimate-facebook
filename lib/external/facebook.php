...
class Facebook
{
  ...
  public static $CURL_OPTS = array(
    ...
    CURLOPT_SSL_VERIFYPEER => false, // Possible workaround 1
  );
        ...
        if (isset($_COOKIE[$cookieName])) {
          ...
          parse_str(trim(
            get_magic_quotes_gpc()
              ...
              : stripslashes($_COOKIE[$cookieName]), // A hack for WordPress
            '"'
          ), $session);
          $session = $this->validateSessionObject($session);
  ...
  protected function makeRequest($url, $params, $ch=null) {
    ...
    if (curl_errno($ch) == 60) { // CURLE_SSL_CACERT
      //self::errorLog('Invalid or no certificate authority found, using bundled information'); // Silence this, workaround #2
